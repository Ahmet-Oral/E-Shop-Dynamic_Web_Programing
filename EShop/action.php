...
if (isset($_POST["category"])) {
    ...
    if (mysqli_num_rows($run_query) > 0) {
        while ($row = mysqli_fetch_array($run_query)) {
            ...
            echo "
					<li><a style='color: white;'href='#' class='category' cid='$cid'>$cat_name</a></li>
			";
        }
        echo "</div>";
    }
...
if (isset($_POST["brand"])) {
    ...
    if (mysqli_num_rows($run_query) > 0) {
        while ($row = mysqli_fetch_array($run_query)) {
            ...
            echo "
					<li><a style='color: white;'href='#' class='selectBrand' bid='$bid'>$brand_name</a></li>
			";
        }
        echo "</div>";
    }
...
// Check connection
    ...
    $product_cat = mysqli_real_escape_string($conn, $_REQUEST['product_cat']);
    $product_brand = mysqli_real_escape_string($conn, $_REQUEST['product_brand']);
    ...
    $product_image = mysqli_real_escape_string($conn, $_REQUEST['product_image']);
    $product_keywords = mysqli_real_escape_string($conn, $_REQUEST['product_keywords']);
...
// Attempt insert query execution
    ...
    if (mysqli_query($conn, $sql)) {
        echo "<div class='alert alert-info'>
						<meta http-equiv='refresh' content='1; URL=homePageGuest.php'>
						<b>Product is added</b>
				</div>";
        exit();
    } else {
        echo "ERROR: Wrong Info $sql. " . mysqli_error($conn);
    }
    $conn->close();
...
// Check connection
    ...
    $product_edit = mysqli_real_escape_string($conn, $_REQUEST['productToEdit']);

    $sql = "DELETE FROM products WHERE product_title=('$product_edit')";
    if (mysqli_query($con, $sql)) {
        header("Location: http://localhost/EShop/removeItem.php");
        alert("Hello World");
        exit();
...
if (isset($_POST["removeItemData"])) {
    $remove_id = $_POST["rid"];
    if (isset($_SESSION["uid"])) {
        $sql = "DELETE FROM products WHERE p_id = '$remove_id' AND user_id = '$_SESSION[uid]'";
    } else {
        $sql = "DELETE FROM products WHERE p_id = '$remove_id' AND ip_add = '$ip_add'";
    }
    if (mysqli_query($con, $sql)) {
...
if (isset($_POST["Common"])) {
    ...
    if (isset($_POST["getCartItem"])) {
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            ?>
            <a style="float:right;" href="cart.php" class="btn btn-warning">Go to Cart&nbsp;&nbsp;<span
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            if (!isset($_SESSION["uid"])) {
                echo '<input type="submit" style="float:right;" name="login_user_with_product" class="btn btn-info btn-lg" value="Ready to Checkout" >
                    ...
                    echo '<div>
                    <a href="payment_success.php"><input type="button" value="BUY" style="float: right; margin-right: 100px; color: black;"/></a>
